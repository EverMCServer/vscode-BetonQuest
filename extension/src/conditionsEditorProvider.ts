...
export class ConditionsEditorProvider implements vscode.CustomTextEditorProvider {

    public static register(context: vscode.ExtensionContext, lspClient: BaseLanguageClient): vscode.Disposable {
        const provider = new ConditionsEditorProvider(context, lspClient);
        const providerRegistration = vscode.window.registerCustomEditorProvider(ConditionsEditorProvider.viewType, provider);
        return providerRegistration;
    }

    private static readonly viewType = 'betonquest.conditionsEditor';
    ...
    public async resolveCustomTextEditor(
        ...
        webviewPanel.webview.html = this.getWebviewContent(
            ...
            [
                ...
                "utils/betonquest", // All other BQ YAML models
            ...
            switch (e.type) {
                ...
                case 'cursor-yaml-path':
                    if (e.content.constructor === Array && e.content.length > 0) {
                        ...
                        offset = 0;
                        const response = await this.lspClient.sendRequest<LocationsResponse>("custom/locations", { yamlPath: ["@conditions", ...e.content], sourceUri: document.uri.toString() } as LocationsParams);
    ...
    private getWebviewContent(webview: vscode.Webview, extensionUri: vscode.Uri, libraries: string[], initialConfig?: InitialConfig): string {
        ...
        const pathReactApp = vscode.Uri.joinPath(extensionUri, "webview", "dist", "conditionsEditor.js");
        // get lib urls
