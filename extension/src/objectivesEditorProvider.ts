...
export class ObjectivesEditorProvider implements vscode.CustomTextEditorProvider {
    ...
    public static register(context: vscode.ExtensionContext, lspClient: BaseLanguageClient): vscode.Disposable {
        const provider = new ObjectivesEditorProvider(context, lspClient);
            ...
            switch (e.type) {
                ...
                case 'cursor-yaml-path':
                    if (e.content.constructor === Array && e.content.length > 0) {
                        ...
                        offset = 0;
                        const response = await this.lspClient.sendRequest<LocationsResponse>("custom/locations", { yamlPath: ["@objectives", ...e.content], sourceUri: document.uri.toString() } as LocationsParams);
    ...
    private getWebviewContent(webview: vscode.Webview, extensionUri: vscode.Uri, libraries: string[], initialConfig?: InitialConfig): string {
        ...
        const pathReactApp = vscode.Uri.joinPath(extensionUri, "webview", "dist", "objectivesEditor.js");
        // get lib urls
