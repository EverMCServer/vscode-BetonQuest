...
export class ConversationEditorProvider implements vscode.CustomTextEditorProvider {
    ...
    public static register(context: vscode.ExtensionContext, lspClient: BaseLanguageClient): vscode.Disposable {
        const provider = new ConversationEditorProvider(context, lspClient);
    ...
    private getWebviewContent(webview: vscode.Webview, extensionUri: vscode.Uri, libraries: string[], initialConfig?: InitialConfig): string {
        ...
        const pathReactApp = vscode.Uri.joinPath(extensionUri, "webview", "dist", "conversationEditor.js");
        // get lib urls
        const pathLibs = libraries.map(lib => webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, "webview", "dist", lib + '.js')));
