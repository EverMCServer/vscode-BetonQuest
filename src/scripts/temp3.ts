...
while ((array1 = pat.exec(listToChange)) !== null) {
    if (array1[0]) {
        let obj = array1[0];
        // console.log(counter++);

        // Get value as key
        const _value = /^        value: (?:\"(.+?)(?<!\\)\"|\'(.+?)(?<!\\)\'),?/m.exec(obj);
        const value = _value?.[1] || _value?.[2] || "";
        const prefix = "betonquest.v2.event."+ value+".";

        // Replace display and description
        obj = obj.replace(/^        display: (?:\".+?(?<!\\)\"|\'.+?(?<!\\)\'),(?:$| +\/\/)/m, `        display: L("${prefix}display"),`);
        obj = obj.replace(/^        description: (?:\".+?(?<!\\)\"|\'.+?(?<!\\)\'),(?:$| +\/\/)/m, `        description: L("${prefix}description"),`);

        let c1 = 0;

        // Replace mandatory / optional args
        ['mandatory', 'optional'].forEach( k => {
        // 1 get mandatory / optional section
        const ms = new RegExp(`^            ${k}: \\[\\n([\\S\\s]+?)^            \\],?$`, "m").exec(obj)?.[1];
        if (ms) {
            let msModified = ms;
            ...
            const patM1 = /^                {\n[\s\S]+?^                },?$\n/gm;
            const patM2 = /^                {.+},?$\n/gm;
            while ((arrayM = patM1.exec(ms) || patM2.exec(ms)) !== null) {
                if (arrayM[0]) {
                    // console.log("c1", c1++);
                    ...
                    // get name
                    const name = / key: (?<!\\)["'`](.+?)(?<!\\)["'`]/i.exec(m)?.[1] || / name: (?<!\\)["'`](.+?)(?<!\\)["'`]/i.exec(m)?.[1].replaceAll(/[^a-zA-Z0-9 ]+/g, '').replaceAll(/(?:^\w|[A-Z]|\b\w)/g, function (word, index) {
                        return index === 0 ? word.toLowerCase() : word.toUpperCase();
                    }).replaceAll(' ', '');
                    const prefixM = prefix + k + "." + name + ".";

                    // console.log(prefixM);

                    // Replace name and tooltip
                    mModified = mModified.replace(/ name: (?:".+?(?<!\\)"|'.+?(?<!\\)')/, ` name: L("${prefixM}name")`);
                    mModified = mModified.replace(/ tooltip: (?:".+?(?<!\\)"|'.+?(?<!\\)')/, ` tooltip: L("${prefixM}tooltip")`);

                    // selection.label
                    let arrayS: RegExpExecArray | null;
                    const patS = /^                            {[\s\S]+?^                            },?/gm;
                    while ((arrayS = patS.exec(ms)) !== null) {
                        if (arrayS[0]) {
                            const s = arrayS[0];
                            let sModified = s;

                            const sValue = /^                                value: '(.+)',?$/m.exec(s)?.[1];
                            const prefixS = prefixM + "option." + sValue;

                            sModified = sModified.replace(/ label: (?:\".+?(?<!\\)\"|\'.+?(?<!\\)\')/, ` label: L("${prefixS}")`);

                            mModified = mModified.replace(s, sModified);
                        }
                    }
        
                    msModified = msModified.replace(m, mModified);
                }
            }

            obj = obj.replace(ms, msModified);
        }
    });
