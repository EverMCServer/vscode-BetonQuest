...
import { Kind, ListElementEditorProps } from "../view/legacyListEditor/components/CommonList/CommonEditor";
import CommonEditor from "../view/legacyListEditor/components/CommonList/CommonEditor";

import { ArgumentsPattern } from "../betonquest/Arguments";

import Default from "../view/eventsEditor/EventsList/EventsEditor/Default";

import BaseLocation from "../view/legacyListEditor/components/CommonList/Input/BaseLocation";
import BlockSelector from "../view/legacyListEditor/components/CommonList/Input/BlockSelector";
import Checkbox from "../view/legacyListEditor/components/CommonList/Input/Checkbox";
import EntityType from "../view/legacyListEditor/components/CommonList/Input/EntityType";
import EntityTypeList from "../view/legacyListEditor/components/CommonList/Input/EntityTypeList";
import Input from "../view/legacyListEditor/components/CommonList/Input/Input";
import InputList from "../view/legacyListEditor/components/CommonList/Input/InputList";
import ItemList from "../view/legacyListEditor/components/CommonList/Input/ItemList";
import Number from "../view/legacyListEditor/components/CommonList/Input/Number";
import PotionEffectTypeList from "../view/legacyListEditor/components/CommonList/Input/PotionEffectTypeList";
import Select from "../view/legacyListEditor/components/CommonList/Input/Select";
import TextArea from "../view/legacyListEditor/components/CommonList/Input/TextArea";
import TextAreaList from "../view/legacyListEditor/components/CommonList/Input/TextAreaList";
import PotionEffectType from "../view/legacyListEditor/components/CommonList/Input/PotionEffectType";

globalThis.locale = "zh-cn";

// String of the list above, for replacing into
let listToChange = [
    {
        value: '*',
        display: '*',
    ...
    },
    {
        ...
        argumentsPattern: {
            ...
            optional: [
                { jsx: Input, name: 'Option Name', key: 'option', type: 'string', placeholder: '(none)', tooltip: 'Jump into a NPC option directely instead of a starting option', config: { allowedPatterns: [/^\S*$/] } },
            ]
        }
    },
    {
    ...
    },
    {
        ...
        argumentsPattern: {
            mandatory: [
                { jsx: Input, name: 'Point Category', type: 'string', defaultValue: 'a_global_point_id_1', placeholder: 'e.g. bonus', tooltip: 'Name of the global point', config: { allowedPatterns: [/^\S*$/] } },
            ]
        }
    },
    {
    ...
    },
    {
        ...
        description: `<><div>Attempts to execute each event if and only if its condition is met, starting from the first onward. It just likes \`if - else if - else if ...\` in any programming languages. Once an event is successfully executed, the rest of the events are skipped.</div><div>e.g.:</div><ul><li>firstExample: "first event1,event2,event3"</li><li>event1: "point carry boxes 10 action:add condition:firstCondition"</li><li>event2: "point carry boxes 20 action:add condition:secondCondition"</li><li>event3: "point carry boxes 40 action:add condition:thirdCondition"</li><ul></>`,
        argumentsPattern: {
            mandatory: [
    ...
    },
    {
        ...
        argumentsPattern: {
            mandatory: [
                { jsx: Input, name: 'Point Category', type: 'string', defaultValue: 'a_point_id_1', placeholder: 'e.g. bonus', tooltip: 'Name of the point', config: { allowedPatterns: [/^\S*$/] } },
                // TODO: multiplication prefix - '*'
                { jsx: Number, name: 'Amount', type: 'float', defaultValue: 0, tooltip: 'amount to change depends on the Action types', allowVariable: true },
            ],
            optional: [
    ...
    },
    {
        ...
        description: `<><div style={{ marginBottom: 8 }}>Runs the specified event(s) player independently (as if it was run from a schedule).</div><div style={{ marginBottom: 8 }}>Note that these events behave differently when executed by this:</div><ul><li>\`tag delete\` - deletes the tag for all players in the database (even if offline)</li><li>\`objective remove\` - removes the objective for all players in the database (even if offline)</li><li>\`journal delete\` - deletes the journal entry for all players in the database (even if offline)</li><li>\`deletepoint\` - clears points of a given category for all players in the database (even if offline)</li></ul></>`,
    ...
    // TODO: ... Or a seprated standalone editor
    {
        ...
        value: 'time',
        display: 'Time',
        description: `<><div style={{ marginBottom: 8 }}>Changes the time of the world.</div><ul><li>\`2.75\` - Set to 2:45 AM</li><li>\`+2.75\` - Add 2.75 hour from now</li><li>\`-2.75\` - Subtract 2.75 hour from now</li></ul></>`,
                ...
                // { jsx: Input, name: 'Vector', key: 'vector', type: 'string', placeholder: 'e.g. (0;0.1;1.3)', tooltip: 'The values of the vector: (x;y;z) for absolute direction, (sideways;upwards;forwards) for relative direction', config: { allowedPatterns: [/^\S*$/] }, allowVariable: true },
                {
                    jsx: Select, name: 'Direction', key: 'direction', type: 'string', placeholder: 'absolute - Absolute', tooltip: 'Coordinate system to be used. Absolute = Coordinate fixed to the world, Relative = Fixed coordinate ralative to the player. Default to `absolute`', config: {
                        options: [
                            ...
                            {
                                label: '`relative_y` - Hrizontal Relative (Relative X & Z, Absolute Y)',
                                value: 'relative_y'
                            },
                            {
                                label: '`relative` - Relative to Player',
                                value: 'relative'
                            },
                        ] as DefaultOptionType[],
                        allowClear: true
...
// Test export list to json for translation
let m = {} as { [key: string]: string };
listToChange.map((e, i) => {
    const p = "betonquest.v1.event." + e.value + ".";
    let r = {
        [p + "display"]: e.display,
        [p + "description"]: e.description,
    } as { [key: string]: string };
    listToChange[i].display = `L("${p + "display"}")`;
    listToChange[i].description = `L("${p + "description"}")`;

    e.argumentsPattern.mandatory.forEach((m, j) => {
        const pm = p + "mandatory." + (m.key || m.name?.toString().replace(/[^a-zA-Z0-9 ]+/g, '').replace(/(?:^\w|[A-Z]|\b\w)/g, function (word, index) {
            return index === 0 ? word.toLowerCase() : word.toUpperCase();
        }).replace(' ', '')) + ".";

        r = {
            ...r,
            [pm + "name"]: m.name!.toString(),
        };
        listToChange[i].argumentsPattern.mandatory[j].name = `L("${pm + "name"}")`;

        if (m.tooltip) {
            r = {
                ...r,
                [pm + "tooltip"]: m.tooltip,
            };
            listToChange[i].argumentsPattern.mandatory[j].tooltip = `L("${pm + "tooltip"}")`;
        }
        if (m.config?.options) {
            m.config?.options.forEach((opt: { label: string, value: string }, k: number) => {
                r = {
                    ...r,
                    [pm + "option." + opt.value]: opt.label
                };
                listToChange[i].argumentsPattern.mandatory[j].config.options[k].label = `L("${pm + "option." + opt.value}")`;
            });
        }
    });

    e.argumentsPattern.optional?.forEach((m, j) => {
        const pm = p + "optional." + (m.key || m.name?.toString().replace(/[^a-zA-Z0-9 ]+/g, '').replace(/(?:^\w|[A-Z]|\b\w)/g, function (word, index) {
            return index === 0 ? word.toLowerCase() : word.toUpperCase();
        }).replace(' ', '')) + ".";
        ...
        if (m.config?.options) {
            m.config?.options.forEach((opt: { label: string, value: string }, k: number) => {
                ...
                opt.label = `L("${pm + "option." + opt.value}")`;
            });
        }
    });
    return r;
